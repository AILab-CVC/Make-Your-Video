...
class TemporalTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head, depth=1, dropout=0., context_dim=None,
                 ...
                 relative_position=False, temporal_length=None, temporal_length_max=64):
        super().__init__()
        ...
        ## CAM implemented here
        if self.causal_attention:
            assert(temporal_length is not None)
            self.mask = torch.zeros([1, temporal_length_max, temporal_length_max])
            for i in range(temporal_length_max//causal_block_size): 
                col_start = i-15
                col_end = col_start+16
                col_start = 0 if col_start < 0 else col_start
                self.mask[:,i,col_start:col_end] = 1
