...
class LatentDepthDiffusion(LatentDiffusion):
    def __init__(self, depth_stage_config, *args, **kwargs):
    ...
    @torch.no_grad()
    def get_batch_depth(self, batch_x, target_size, temporal_norm=False):
        # midas.blocks L335 may encounter error when using large batch size 
        b, c, t, h, w = batch_x.shape
        x = rearrange(batch_x, 'b c t h w -> (b t) c h w')
        ...
        if temporal_norm:
            ...
            depth_min, depth_max = torch.amin(cond_depth, dim=[1, 2, 3, 4], keepdim=True), torch.amax(cond_depth, dim=[1, 2, 3, 4], keepdim=True)
